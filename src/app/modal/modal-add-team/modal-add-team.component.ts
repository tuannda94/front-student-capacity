...
export class ModalAddTeamComponent implements OnInit {
  ...
  addTeam() {
    ...
    if (dataTeam.name.length < 30) {
      ...
      this.toast.error({ summary: "Tên nhóm của bạn quá dài !!!", detail: "Lỗi", duration: 2000 });
    }
  }
  ...
  editTeam() {
    ...
    if (dataTeam.name.length < 30) {
      ...
      if (dataTeam.name === this.teamDetail.name && !this.isEditData) {
        ...
        formDataTeam.append("contest_id", this.data.contest_id);
        formDataTeam.append("user_id", this.user_id);
        this.teamService.editTeam(formDataTeam, this.teamDetail.id).subscribe((res) => {
          if (!res.status) {
            this.toast.error({ summary: res.payload, detail: "Lỗi", duration: 2000 });
            this.dialogRef.close(true);
          } else {
