...
export class ContestAsideComponent implements OnInit {
  ...
  constructor(
    ...
    private isPopup: LocalStorageService,
  ) {}

  ngOnInit(): void {}

  runTime() {
    this.date_start = new Date(moment(this.contestDetail.date_start).format("lll")).getTime();
    this.date_end = new Date(moment(this.contestDetail.register_deadline).format("lll")).getTime();
    this.date_register_start = new Date(moment(this.contestDetail.start_register_time).format("lll")).getTime();

    this.date_register_start > this.today ? (this.disabled = false) : this.disabled;

    setInterval(() => {
      this.date_register_end = new Date(moment(this.contestDetail.end_register_time).format("lll")).getTime();
      this.today = new Date().getTime();
      let distance = this.date_register_end - this.today;

      if (distance < 0 || this.contestDetail.status == 2 || this.date_register_start > this.today) {
        this.disabled = false;
  ...
  checkStatusContest(item: Contest): any {
    ...
    if (item.status <= 1) {
      if (this.date_register_start > this.today) {
        ...
        result = item.status_user_has_join_contest ? "Đang mở đăng ký" : "Đăng ký";
      } else if (this.date_start > this.today) {
        result = "Đã đóng đăng ký";
      } else if (this.date_end > this.today) {
        result = item.status_user_has_join_contest ? "Đã tham gia" : "Đang diễn ra";
  ...
  classNameWhereStatus(item: Contest): any {
    ...
    if (item.status <= 1) {
      if (this.date_register_start > this.today) {
        ...
        result = item.status_user_has_join_contest ? "my-btn-disabled my-btn-s" : "my-btn-secondary my-btn-s";
      } else if (this.date_start > this.today) {
        result = "my-btn-disabled my-btn-s";
      } else if (this.date_end > this.today) {
        result = item.status_user_has_join_contest ? "my-btn-disabled my-btn-s" : "my-btn-disabled my-btn-s";
      } else {
        result = "my-btn-disabled my-btn-s";
      }
    } else {
